...
void PrintCacheStatsRecord(std::string_view cache_name,
  ...
  if (output_format == OutputFormat::CSV) {
    std::cout << cache_name << kCsvFieldSeparator << cache.Capacity()
              ...
              << cache.GetStats().hits << kCsvFieldSeparator
              << cache.GetStats().misses << kCsvFieldSeparator
              << cache.PutStats().hits << kCsvFieldSeparator
              << cache.PutStats().misses << kCsvFieldSeparator << throughput;
    ...
    std::cout << '{' << R"("name": ")" << cache_name << R"(", "capacity":)"
              ...
              << R"(, "read_hits": )" << cache.GetStats().hits
              << R"(, "read_misses": )" << cache.GetStats().misses
              << R"(, "write_hits": )" << cache.PutStats().hits
              << R"(, "write_misses": )" << cache.PutStats().misses
...
void RunCacheBenchmarkCases(std::string_view cache_name,
  ...
  for (auto const cache_capacity :
       std::array<size_t, 4>{100, 1'000, 10'000, 100'000}) {
    // first reader config then writer config
    for (auto const &configs : std::array<std::pair<Config, Config>, 2>{
             {{{cache_capacity * 5, 10, 500'000, 0ns},
               {cache_capacity * 2, 2, 100'000, 0ns}},
              {{cache_capacity * 5, 2, 500'000, 0ns},
               {cache_capacity * 2, 10, 100'000, 0ns}}}}) {
